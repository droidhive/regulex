...
define(['./Kit', './parse'], function (K, parse) {
  ...
  var colorPallate = {
    "blue": "#0499c0", // blue
    "yellow": "#f8b936", // - yellow
    "pink": "#f35f6f", //- pink
    "indigo": "#074f68", //- dark blue
    "green": "#c2ca24", //- green
    "greyLine": "#cdcec7", //- grey line color
    "bgColor": "#ebf0e5", //- grey background color
    "white": "white"
  }
  ...
  */
  function visualize(re, flags, paper) {
    ...
    bg.attr("fill", colorPallate.bgColor);
    bg.attr("stroke", colorPallate.bgColor);
  ...
  function textLabel(x, y, s, color) { // x is center x ,y is bottom y
    ...
    var textWidth;
    if (lines.length > 1) {
      textWidth = Math.max.apply(Math, lines.map(function (a) {
        return a.length
      }));
    } else {
      textWidth = s.length;
  ...
  var plotNode = {
    startPoint: function (node, x, y) {
      return point(x, y, colorPallate.green)
    },
    endPoint: function (node, x, y) {
      return point(x, y, colorPallate.pink)
    },
    ...
    dot: function (node, x, y) {
      var bgColor = colorPallate.green,
        textColor = 'white';
    ...
    charset: function (node, x, y) {
      ...
      var charBgColor = colorPallate.blue,
        charTextColor = colorPallate.black,
        clsBgColor = colorPallate.indigo,
        clsTextColor = colorPallate.white,
        rangeBgColor = colorPallate.pink,
    ...
    assert: function (node, x, y) {
      ...
      var conf, nat = node.assertionType,
